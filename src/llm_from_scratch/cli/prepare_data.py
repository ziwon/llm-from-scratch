...
from ..utils.logging import console, setup_logger
...
def prepare(
    ...
    if 0 < train_split < 1:
        ...
        with open(train_file, "w", encoding="utf-8") as f:
            f.write(train_text)
        console.print(
            f"[green]✓[/green] Saved training data: {train_file} ({len(train_text):,} chars)"
        )
        ...
        with open(val_file, "w", encoding="utf-8") as f:
            f.write(val_text)
        console.print(
            f"[green]✓[/green] Saved validation data: {val_file} ({len(val_text):,} chars)"
        )
    else:
        # Save single file
