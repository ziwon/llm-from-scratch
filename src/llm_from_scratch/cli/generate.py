...
def generate(
    ...
    max_tokens: int = typer.Option(100, "--max-tokens", "-m", help="Maximum tokens to generate"),
    ...
    repetition_penalty: float = typer.Option(1.0, "--repetition-penalty", help="Repetition penalty"),
    num_samples: int = typer.Option(1, "--num-samples", "-n", help="Number of samples to generate"),
    ...
    if 'config' in checkpoint:
        ...
        console.print("[red]Error: No config found in checkpoint and no config file specified[/red]")
        raise typer.Exit(1)
    ...
    # Single prompt mode
        ...
        if not prompt:
            console.print("[red]Error: Please provide a prompt with --prompt or use --interactive mode[/red]")
            raise typer.Exit(1)
