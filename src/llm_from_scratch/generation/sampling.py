...
def top_k_sampling(logits: torch.Tensor, k: int) -> torch.Tensor:
    ...
    return torch.where(logits < min_value, torch.tensor(float('-inf')).to(logits.device), logits)
...
def top_p_sampling(logits: torch.Tensor, p: float) -> torch.Tensor:
    ...
    return torch.where(indices_to_remove, torch.tensor(float('-inf')).to(logits.device), logits)
