...
from llm_from_scratch.core import GPTDataset, TokenizerWrapper, create_dataloader
...
def test_dataloader_creation(sample_text_file):
    ...
    config = DataConfig(
        train_file=str(sample_text_file), max_length=32, stride=4, cache_dir=None
    )

    dataloader = create_dataloader(config=config, batch_size=8, shuffle=True)
