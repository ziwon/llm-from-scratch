...
class Trainer:
    ...
    def __init__(
        ...
        self.optimizer = create_optimizer(self.model, config.optimizer, config.training)
        ...
        self.scheduler = create_scheduler(
            self.optimizer, config.scheduler, config.training, self.total_steps
        )
    ...
    def train_epoch(self) -> float:
        ...
        for batch_idx, (input_ids, target_ids) in enumerate(progress_bar):
            ...
            progress_bar.set_postfix(
                {"loss": f"{loss.item():.4f}", "lr": f"{self.get_lr():.2e}"}
            )

            # Log to tensorboard
            if (
                self.writer
                and self.global_step % self.config.training.log_interval == 0
            ):
                self.writer.add_scalar("train/loss", loss.item(), self.global_step)
                self.writer.add_scalar("train/lr", self.get_lr(), self.global_step)
                self.writer.add_scalar(
                    "train/perplexity", torch.exp(loss).item(), self.global_step
                )
